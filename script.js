document.addEventListener('DOMContentLoaded', function() {
  ...
  function renderTransactions() {
    ...
    transactions.forEach(tx => {
      ...
      details.innerHTML = `<span>${tx.date}</span>
                           <span>${tx.type === 'income' ? 'Bevétel' : (tx.type === 'expense' ? 'Kiadás' : 'Megtakarítás')}</span>
  ...
  exportCSVBtn.addEventListener('click', function() {
    ...
    transactions.forEach(tx => {
      const line = `${tx.date};${tx.type};${getCategoryIcon(tx.category)} ${getCategoryName(tx.category)};${tx.amount}`;
      csvContent += line + "\n";
    });
    ...
    let totalExpense = transactions.filter(t => t.type === 'expense')
                                  .reduce((a, b) => a + Number(b.amount), 0);
    let totalSaving = transactions.filter(t => t.type === 'saving')
                                 .reduce((a, b) => a + Number(b.amount), 0);
    csvContent += "\n--- Összesítés ---\n";
  ...
  // Diagram frissítése a tranzakciók alapján (csak kiadások kategóriák szerint)
